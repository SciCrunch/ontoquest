...
public class OntGraphTest extends OntoquestTestAdapter {
  ...
  public void testGetSubclasses() throws OntoquestException {
    ...
    OntGraph g = OntGraph.get(id, OntGraph.NeighborType.SUBCLASSES, kbid, attributes, OntGraph.InputType.OID,context);
...
//    String name = "CHEBI_23367";
    ...
    OntGraph g = OntGraph.get(name, OntGraph.NeighborType.SUBCLASSES, kbid, attributes, OntGraph.InputType.NAME,context);
//    ClassNode node = ClassNode.get(id, context);
//    assertTrue(g.getNodes().size() > 0);
    assertTrue(g.getEdges().size() > 0); 
    for (Relationship r : g.getEdges()) {
  ...
  public void testGetSuperclasses() throws OntoquestException {
    attributes.put("level", 3);
    OntGraph g = OntGraph.get("cerebellum", OntGraph.NeighborType.SUPERCLASSES, kbid, attributes, OntGraph.InputType.TERM,context);
//    ClassNode node = ClassNode.get(id, context);
//    assertTrue(g.getNodes().size() > 0);
    assertTrue(g.getEdges().size() > 0); 
    for (Relationship r : g.getEdges()) {
  ...
  public void testGetParts() throws OntoquestException {
    ...
    OntGraph g = OntGraph.get(name, OntGraph.NeighborType.PARTS, kbid, attributes, OntGraph.InputType.NAME, context);
//    assertTrue(g.getNodes().size() > 0);
    assertTrue(g.getEdges().size() > 0);
    for (Relationship r : g.getEdges()) {
      System.out.println(r.getLabel1() + " " + r.getPname() + " " + r.getLabel2());
...
//      assertTrue(g.getNodes().size() > 0);
  ...
  public void testGetWhole() throws OntoquestException {
    ...
    attributes.put("level", 3);
    OntGraph g = OntGraph.get(name, OntGraph.NeighborType.WHOLE, kbid, attributes, OntGraph.InputType.NAME, context);
//    assertTrue(g.getNodes().size() > 0);
    assertTrue(g.getEdges().size() > 0);
    for (Relationship r : g.getEdges()) {
      System.out.println(r.getLabel1() + " " + r.getPname() + " " + r.getLabel2());
