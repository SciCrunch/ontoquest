...
/**
 * @version $Id: DeprecatedOntGraphTest.java,v 1.1 2010-10-28 06:30:35 xqian Exp $
 *
 */
public class DeprecatedOntGraphTest extends OntoquestTestAdapter {
  ...
  public void testGetSubclasses() throws OntoquestException {
    ...
    DeprecatedOntGraph g = DeprecatedOntGraph.get(id, OntGraph.NeighborType.SUBCLASSES, kbid, attributes, OntGraph.InputType.OID,context);
...
//    attributes.put("level", 3);
    ...
    DeprecatedOntGraph g = DeprecatedOntGraph.get(name, OntGraph.NeighborType.SUBCLASSES, kbid, attributes, OntGraph.InputType.NAME,context);
//    ClassNode node = ClassNode.get(id, context);
//    assertTrue(g.getNodes().size() > 0);
    assertTrue(g.getEdges().size() > 0); 
    for (Relationship r : g.getEdges()) {
  ...
  public void testGetSuperclasses() throws OntoquestException {
    attributes.put("level", 3);
    DeprecatedOntGraph g = DeprecatedOntGraph.get("cerebellum", OntGraph.NeighborType.SUPERCLASSES, kbid, attributes, OntGraph.InputType.TERM,context);
//    ClassNode node = ClassNode.get(id, context);
//    assertTrue(g.getNodes().size() > 0);
    assertTrue(g.getEdges().size() > 0); 
    for (Relationship r : g.getEdges()) {
  ...
  public void testGetParts() throws OntoquestException {
    ...
    attributes.put("level", 3);
    DeprecatedOntGraph g = DeprecatedOntGraph.get(name, OntGraph.NeighborType.PARTS, kbid, attributes, OntGraph.InputType.NAME, context);
//    assertTrue(g.getNodes().size() > 0);
    assertTrue(g.getEdges().size() > 0);
    for (Relationship r : g.getEdges()) {
      System.out.println(r.getLabel1() + " " + r.getPname() + " " + r.getLabel2());
  ...
  public void testGetParentsByID() throws OntoquestException {
    ...
    while (rs.next()) {
      ...
      String id = rid + "-" + rtid;
      DeprecatedOntGraph g = DeprecatedOntGraph.get(id, OntGraph.NeighborType.PARENTS, kbid, attributes, 
                                OntGraph.InputType.OID, context);
//      assertTrue(g.getNodes().size() > 0);
      for (Relationship r : g.getEdges()) {
        System.out.println(r.getLabel1() + " " + r.getPname() + " " + r.getLabel2());
      }
  ...
  public void testGetWhole() throws OntoquestException {
    ...
    attributes.put("level", 3);
    DeprecatedOntGraph g = DeprecatedOntGraph.get(name, OntGraph.NeighborType.WHOLE, kbid, attributes, OntGraph.InputType.NAME, context);
//    assertTrue(g.getNodes().size() > 0);
    assertTrue(g.getEdges().size() > 0);
    for (Relationship r : g.getEdges()) {
      System.out.println(r.getLabel1() + " " + r.getPname() + " " + r.getLabel2());
